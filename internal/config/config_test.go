...
func TestLoadKeepsExplicitCloudflareDNSRecordsOnly(t *testing.T) {
	...
	if len(cfg.CloudflareDNS.RecordSets) != 1 {
		t.Fatalf("expected only explicit record sets, got %d: %#v", len(cfg.CloudflareDNS.RecordSets), cfg.CloudflareDNS.RecordSets)
	}
	records := cfg.CloudflareDNS.CarrierRegionRecords("cu")
	if len(records) != 1 || records[0].Domain != "custom-us.example.com" {
		t.Fatalf("custom CU US record was not preserved: %#v", records)
	}
	ctRecords := cfg.CloudflareDNS.CarrierRegionRecords("ct")
	if len(ctRecords) != 0 {
		t.Fatalf("unexpected generated CT records: %#v", ctRecords)
	}
}
